...
<body class="example">
  ...
  @endauth
   ...
   <div class="col-lg-9">
	<header>
		<p><h1>{{$content['title']}}</h1></p>
		<p>Автор:{{ $user->name }}</p>
	</header>
	
	<div class="portrait"><img src="{{ URL::asset($content['photo']) }}", alt="roki">
    </div>
 	@auth   
    @if (Auth::user()->can('update-photo', $model))
    
    <div class="form-inline form-group mx-sm-3">
	<form class="form-group mx-sm-3" method="GET" action="{{ route('conTest.edit', ['contr' => $content['id']]) }}">
	@csrf	
	<input class="btn btn-primary mb-2" type="submit" name="knopka" value="Редактировать">
	</form>
	<form method="POST" action="{{ route('conTest.destroy', ['contr' => $content['id']]) }}">
	@method('DELETE')
	@csrf	
	<input class="btn btn-primary mb-2" type="submit" name="knopka" value="Удалить">
	</form>
	</div>
	@endif
	@endauth
